...
export default async function HomePage({ searchParams }: PageProps<"/">) {
  ...
  const [summary, upcoming, billings, breakdown, recent] = await Promise.all([
    getMonthlySummary(household.id, yearMonth, household.monthStartDay),
    getUpcomingFixed(household.id, 10),
    getCardBillings(household.id, yearMonth),
    getCategoryBreakdown(household.id, yearMonth, household.monthStartDay),
  ...
  const billingsWithAmount = billings.filter((item) => item.total > 0);
  const totalCardBilling = billingsWithAmount.reduce(
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted">
                        {item.dueDate.getMonth() + 1}월{" "}
                        {item.dueDate.getDate()}일
          ...
          trailing={
            totalCardBilling > 0 ? (
              <span className="tabular text-sm font-bold">
                {formatWon(totalCardBilling)}
              </span>
            ) : undefined
          }
        >
          {billingsWithAmount.length === 0 ? (
            <EmptyHint
            ...
            <ul className="space-y-3">
              {billingsWithAmount.map(({ card, total, installment, period }) => (
